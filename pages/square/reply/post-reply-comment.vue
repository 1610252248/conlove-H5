...
export default {
	...
	methods: {
		...
		// 拿数据
		getComment(id) {
			this.$http.get('/post/getReply', { id }).then(res => {
				this.comment = res.data;
				this.setAppreciate(this.comment.childComments);
			});
		},
		// 用户点赞评论列表
		setAppreciate(comments) {
			this.comment.childComments = [];
			this.$http.get('/post/reply/commentAppreciate').then(res => {
		...
		changLike(id, isLike) {
			if (!isLike) {
				this.$http.post('/post/reply/addCommentLike', { commentId: id });
			} else {
				this.$http.get('/post/reply/delCommentLike', { id });
			}
		}
	}
};
